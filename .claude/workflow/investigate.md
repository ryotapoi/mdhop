...
- 机上で分からない挙動はコード読みを続けず、`bin/mdhop <args>` を実行して挙動を見るか、ユーザーに再現手順を確認する
...
- 調査結果が将来も効くなら、特定ソースに紐づく知見はそのコードのコメントへ、横断的な挙動は `llm-wiki/` の該当地図へ。単一の集約ファイルは作らない。要求や粒度が変わるなら `backlog/backlog.md` に記録する
