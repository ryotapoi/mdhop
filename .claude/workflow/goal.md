...
この workflow は mdhop の Goal 手順の正本。実装作業の発火入口は `goal-workflow` skill とし、`goal-workflow` skill はこのファイルを読んで進める。
...
- 各レビュー単位につき Codex レビュー実行は 3 回を目安とする。超過しても止まらず収束まで続け、超過の事実（実行回数・要因となった指摘・収束結果）を記録して Goal 完了報告の `レビュー上限超過` で通知する。
...
- レビューループ（commit 内のレビュー周回、Goal Review の Codex 実行回数）が上限の目安を超過した場合は、Goal 完了時に `レビュー上限超過` として対象単位・回数・要因となった指摘・収束結果を提示する。超過がない場合も `レビュー上限超過: なし` と明示する。
