...
- 複数コミットやマイルストーン（v0.6, v0.7 など）の区切り
- 同じ種類の修正が続いている
...
- `docs/rules/architecture.md`, `llm-wiki/`（作業地図）
...
- `llm-wiki/` の地図健全性: `wiki-lint` スキル（孤立・リンク切れ・sources 切れの機械検証＋「速い / docs レベルでない / 嘘がない / 拾える」の不変条件照合）
