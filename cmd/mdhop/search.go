...
func runSearch(args []string) error {
	...
	fields := fs.String("fields", "", "comma-separated fields to output (meta)")
	sortKey := fs.String("sort", "", "sort by meta key (prefix with - for desc)")
	...
	if err := validateFields(fieldList, validSearchFieldsCLI, "search"); err != nil {
		return err
	}
	...
	switch *format {
	case "json":
		return printSearchJSON(os.Stdout, result)
	default:
