...
const moveHelp = `Usage: mdhop move --from <path> (--to <path>|--to-template <template>) [--vault <path>] [--format json|text]
...
Options:
  ...
  --to-template <template>
                      Destination template for single-note moves. Supports {field}, {field|fallback}, {dateField:year}, and {basename}.
  --vault <path>      Optional. Vault root directory. Default: ".".
  --format json|text  Optional. Output format. Default: text.
...
Behavior notes:
  ...
  --to-template is single-file note mode only; it is incompatible with --to and directory moves.
  Template fields are read from indexed source-note frontmatter. Missing fields without fallback, fields with multiple values, invalid date year extraction, and empty or vault-escaping expanded destinations fail before Move changes files.
