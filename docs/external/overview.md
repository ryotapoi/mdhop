...
- **basename 衝突**: 拡張子を除いたファイル名（basename）が複数存在する状態。
- 厳密モードでは、**曖昧な解決が発生する状態を禁止**する。
  - `[[a]]` と `[x](a.md)` は同一扱いで、basename が複数ある場合は曖昧とみなす。
  - 曖昧なリンクの追加や、衝突を生む追加/移動はエラー。
...
- `build`
  - 必須: なし
  - 任意: `--vault`
  - 補足: basename 衝突がある場合は **エラー**（厳密モード）
- `update`
  - 必須: `--file`（複数回指定可）
  - 任意: `--vault`
  - 補足: 更新後の内容に、曖昧解決が発生するリンクが含まれる場合は **エラー**
    - 対象: `[[a]]` / `[x](a.md)` など basename 解決が必要なリンク
- `add`
  - 必須: `--file`（複数回指定可）
  - 任意: `--vault`
  - 補足: 既存ファイルが指定された場合はエラー
  - 補足: basename 衝突が発生する追加は **エラー**
