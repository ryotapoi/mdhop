...
## 2026-09-06: dry backend の結果型を簡約

共通ディスパッチの `int64` 結果に合わせるためだけに、dry backend が一時 ID の採番とパスとの双方向変換を維持していた。利用側が必要とするのは解決後のパスであり、ID の同一性や保存は要求されていない。

ディスパッチの共有は維持し、既存 backend interface の結果型だけを型パラメータにした。これにより DB / build は実 node ID、dry 検証はパスを直接返せる。dry 検証を別のディスパッチへ分ける案は、解決順序の同期負担を戻すため採らなかった。現在の呼び出し関係は `link_resolver.go` と `frontmatter_path_guard.go` を参照する。
