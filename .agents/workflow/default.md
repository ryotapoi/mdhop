...
- **Constraints**:
  ...
  - 判断に影響する `docs/rules/`, `docs/decisions/`, `backlog/backlog.md`, `references/knowledge.md`, `docs/specs/`（あれば）は推測で済ませず実物を確認する。
...
- **High-risk**: SQLite スキーマ、SQL 意味変更、リンク解決、ルート優先ルール、stdout JSON、vault escape、`move` / `delete --rm` 等の破壊的処理、公開 API、外部連携、並行性
