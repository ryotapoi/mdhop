...
regen: none
sources:
  # プロジェクト内ソースからは復元できない外部由来・実測・実装の非対称の記録。
  # 各項目の根拠（外部ライブラリの挙動・エッジケースの実測・テストの罠）はソースに無い。
---

# Knowledge Base

mdhop 固有の技術的な知見・ハマりどころを集約する場所。`regen: none`（外部知見）として llm-wiki/ に属する。ソースを読んでも復元できない外部ライブラリの罠・エッジケースの実測・実装上の非対称を残す。正本（docs/・ソース・テスト）と矛盾したら正本が勝つ（→ `docs/rules/information-management.md`）。

## このファイルの使い方

- **いつ読むか**: 新しい機能の実装前、バグ調査時に関連セクションを確認する
- **何を書くか**: ソースから復元できない知見（罠、回避策、仕様の癖、実測値など）。ソースを読めば分かることは書かない
- **CLAUDE.md との違い**: CLAUDE.md は毎回読み込まれる目次。ここは「該当する実装のときだけ必要な知見」
- **書き方**: セクション見出しでテーマ分け。各項目は簡潔に。症状・原因・対処がわかるように書く
- **昇格**: ここの知見が設計判断や仕様を拘束し始めたら docs/decisions か docs/specs へ昇格させる（llm-wiki/ に正本級の情報を溜めない）
