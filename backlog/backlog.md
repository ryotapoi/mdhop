...
- [ ] 01-concept.md の diagnose 記述と overview.md の食い違い解消（2026-07-25、01-concept.md の用語更新 v0.16.6 の follow-up）: `docs/rules/01-concept.md:91` は `mdhop diagnose : basename衝突、phantom一覧、パース失敗等を検出` と書くが、正本 `docs/specs/overview.md:77` の diagnose 記述には「パース失敗」の検出が挙がっていない。実装が実際にパース失敗を報告するか確認したうえで整合させる
- [ ] 01-concept.md §3.2 の core / mutate 分類のずれ整理（2026-07-25、01-concept.md の用語更新 v0.16.6 の follow-up）: §3.2「読み取り系と書き換え系を概念分離」の一覧（mutate 一覧は v0.16.6 の 01-concept.md 更新で `move/disambiguate/simplify/repair/convert` に更新済み）と §5「コマンドの全体像」の記述がずれている。`add` は §5 では mutate 例として挙がるが §3.2 の mutate 一覧に含まれない。`update` はインデックス DB のみを書く点で `build` と同格だが（`internal/core/update.go` に vault ファイルへの書き込みはない）、§3.2 では `build` と分けて別扱いすべきかも未確定。加えて、Git 差分に影響しうる書き込み経路が §3.2 のどちらの一覧にも登場せず複数抜けている疑いがある: 無条件で vault ファイルを書き換える `set`（`internal/core/set.go:91` の `writeFilePreservePerm`）、`--rm` 指定時に vault ファイルを削除する `delete`（`internal/core/delete.go:13` の `RemoveFiles`、`delete.go:84` の `os.Remove`）、`--write` 指定時に `mdhop.yaml` を書き換える `init-meta`（`cmd/mdhop/init_meta.go:54` の分岐、57行目の `os.WriteFile`）。core 側は「安全に読むだけ」と明記されているため、これは末尾の「など」で流せる軽微な列挙漏れではなく、書き込み経路の一群がまとめて欠落している可能性がある。分類の意図を確定して整合させる
- [ ] 01-concept.md:24 の `rename` 表記を直すかの確定（2026-07-25、01-concept.md の用語更新 v0.16.6 の follow-up）: `docs/rules/01-concept.md:24` の「rename/move が発生したとき、リンク整合性・「最短リンク表記」を維持するのが難しい」という行の扱いが未決。現行コマンドに `rename` は存在せず `move` のみだが、この行はコマンド列挙ではなくユーザー側のイベント（ファイル名変更が起きたとき）を指している可能性がある。Goal Review で判定が割れた: 他系統 reviewer（Codex）は同一ファイル内の貫通漏れとして `rename` を直すべきと判断、自系統 reviewer（fable）はイベント記述であり許容範囲と判断。語をコマンド語彙として直すか、イベント記述として原文のまま残すかを確定する
