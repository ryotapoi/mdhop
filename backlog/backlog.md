...
- [ ] convert / repair / simplify / init-meta / search(text) の CLI・format テスト追加（`format_repair.go` / `format_simplify.go` / `format_convert.go` / `printSearchText` が 0% カバレッジ。v0.10.0 受け入れ条件「既存出力の互換性を壊さない」の検証基盤。既存 cli_test.go / format_test.go の規約に乗せる。1〜2 commit）
