...
- [x] PKM 整理向け孤立検出フラグ（`search` への 3 フラグ追加、まとめて 1 タスク）
  ...
  - **分割しない理由**: 3 フラグとも `internal/core/search.go` と `search_test.go` で完結。SQL は `NOT EXISTS` 条件を共有できるため、分割すると共通部分の重複ロードが発生する
  - **影響範囲**: search コマンドのフラグ追加 + SQL クエリ拡張（`NOT EXISTS` 系）+ fixture と 3 ケースのテスト追加
